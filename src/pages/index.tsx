  ...
  return (
    <div>
      ...
      <div style={{marginTop:"25px"}}>
        ...
        <input placeholder="Movie title..." type="checkbox" checked={hasOscar} onChange={(e) => setHasOscar(e.target.checked)}/>
        <label htmlFor="">Received an Oscar</label>
        <button onClick={handleSubmit}>Submit Movie</button>
      </div>
        {movieList && movieList.map((movie:{ id: string; title: string; releaseDate: number; receivedOscar: boolean })=> (
          <div key={movie.id}>
        ...
        <div>
          <input type="file" onChange={(e)=> setFileUpload(e ? Array.from(e.target?.files || []) : null)}/>
